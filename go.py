...
def scan_file(filepath):
    ...
    try:
        ...
        if ext.endswith(".apk"):
            results.append(("APK", "Android APK (keystore detection)"))
        if ext.endswith(".html") or ext.endswith(".htm"):
            results.append(("HTML", "Scanning HTML for hashes/encryption"))

        # Scan isi file text lebih kuat (regex hash & base64)
        with open(filepath, "rb") as f:
            content = f.read()
            try:
                text = content.decode(errors="ignore")

                # Deteksi hash heksadesimal
                for match in re.findall(r"\b[a-fA-F0-9]{16,128}\b", text):
                    htype = detect_hash_type(match)
                    if htype:
                        results.append((htype, match))

                # Deteksi string base64
                for match in re.findall(r"[A-Za-z0-9+/=]{20,}", text):
                    if len(match) % 4 == 0:
                        results.append(("BASE64", match))

                # Deteksi AES/Blowfish/Cipher berbasis hex panjang
                for match in re.findall(r"\b[0-9A-F]{32,512}\b", text):
                    if match not in [r[1] for r in results]:
                        results.append(("CIPHERTEXT", match))
...
def main():
    ...
    for i,r in enumerate(results,1): 
        preview = r[1] if isinstance(r[1], str) else str(r[1])
        print(f"{i}. {r[0]} -> {preview[:60]}...")
